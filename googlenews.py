...
class GoogleNews:
    ...
    def search_by_keywords_or(self, keywords_query: str, k: int = 100, 
        ...
        # 신뢰할 수 있는 언론사가 지정된 경우 해당 언론사에서만 검색
        if trusted_press:
            return self._search_trusted_press_or(keywords_query, k, trusted_press)
        else:
            # 전체 언론사에서 OR 검색
    ...
    def _search_trusted_press_or(self, keywords_query: str, k: int, 
                                trusted_press: Dict) -> List[Dict[str, str]]:
        """
        신뢰할 수 있는 언론사에서 OR 조건으로 뉴스 검색
        ...
        # 각 신뢰할 수 있는 언론사별로 OR 검색
        for press_name, aliases in trusted_press.items():
            print(f"언론사 '{press_name}'에서 OR 검색 중...")
            
            for alias in aliases:
                try:
                    # 언론사별 OR 검색 URL 생성 (site: 제한자 사용)
                    search_query = f"({keywords_query}) site:{alias}"
                    encoded_query = quote(search_query)
                    url = f"{self.base_url}/search?q={encoded_query}&hl=ko&gl=KR&ceid=KR:ko"
        ...
        # 최대 개수 제한
        final_results = all_results[:k]
        print(f"신뢰할 수 있는 언론사에서 OR 검색으로 총 {len(final_results)}개 뉴스 수집 완료")
