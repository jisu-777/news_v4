...
**중복 처리 원칙**
- 중복 내용 기사 최대한 제외
- 완전히 동일 제목(문자 단위 동일) 또는 네이버 oid/aid 동일이면 1건만 유지
- 동일한 이슈를 여러 언론이 보도한 경우:
  - 우선순위: 조선일보/중앙일보/동아일보 > 한국경제/매일경제 > 연합뉴스 > 기타 경제지
  - 단, 신뢰 높은 다른 언론사가 더 자세한 맥락·인용을 담았다면 함께 포함

**포함(Y)**
- 재무/실적 (매출, 영업이익, 순이익, 배당, 투자계획)
- 회계/감사 (회계처리 변경, 감사의견, 감리 결과, 내부회계관리제도)
- 기업 구조/가치 변동 (M&A, 자회사 설립·매각, 지분 변동, 신규사업, 전략 방향성)
