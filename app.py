...
**중복 제로 원칙**
- 같은 이슈에서 여러 매체/중복 제목이 있을 경우, 최종 결과에는 그 이슈당 1건만 남긴다.
- URL 기반 판정: originallink가 있으면 그 도메인/경로를 우선 사용해 동일 이슈 여부를 판단한다.
- 네이버 기사(news.naver.com, n.news.naver.com, mnews.naver.com)는 article/<oid>/<aid> 조합이 같으면 같은 이슈로 본다.
- 제목/내용 기반 판정: 제목에서 대괄호·공백 중복·따옴표·이모지·특수문자를 제거하고, 영문은 소문자화한다.
- 날짜·수치(%, 원, 억/조 단위)·"단독/속보/영상/종합" 등의 군더더기는 제거한다.
- 이렇게 정규화한 제목끼리 의미가 동일하거나 핵심명사(회사/사건/역할) 조합 + 동일 시점(같은 발표/행사/사건 날짜) 이 겹치면 같은 이슈로 본다.

**이슈당 1건 선택 기준 (타이브레이커)**
- 기사 품질: 수치·인용·맥락 설명이 더 풍부한 기사 우선
- 출처 신뢰도(우선순위): 조선일보 > 중앙일보 > 동아일보 > 한국경제 > 매일경제 > 연합뉴스 > (이데일리/아시아경제/뉴스핌/뉴시스/헤럴드경제/더벨/비즈니스포스트/머니투데이 등)
- 최신성: 최초 단독보다 가장 최신이면서 정보가 더 풍부한 정리 기사를 우선
- 위 기준이 비슷하면 **원문(출처 표기 명확)**을 우선

**포함(Y) 기준**
- 삼일PwC/회계법인 자체가 주제이거나, 해당 사건에서 핵심 역할(매각주관·자문·감정·보고서·발표 등)을 담당
- 삼일PwC의 보고서/감정이 기사 핵심 근거로 사용
- 컨소시엄 참여가 명시(역할/명단 포함)
- 사설·칼럼이라도 삼일PwC 관련 핵심 내용을 다룸

**제외(N) 기준**
- 단순 언급/경력 소개/배경 문장 수준
- 주제가 회계법인과 직접 관련 없음
- 광고성/스폰서/외국어 기사, 중복 보도(이슈당 1건 원칙으로 정리)

**중복 검증 체크리스트**
- 같은 이슈로 묶이는 기사들이 있는가? 있다면 최고 1건만 남겼는가?
- 같은 이슈를 다른 매체로 두 번 이상 내보내지 않았는가?
- 제목만 다르고 핵심 내용이 동일한 기사들을 병합했는가?

**중요**: 
- **무조건 1개 이상의 뉴스를 반드시 선별해야 합니다.** 1개 미만으로 선별하면 안됩니다.
- 가능하면 7-10개까지 선별하되, 같은 이슈 중복 금지 원칙을 지켜 서로 다른 이슈만 담으세요.
- 내용 중복·이슈 중복 모두 금지.
...
"""
            ...
            # 경쟁사 전용 상세 프롬프트
            analysis_prompt = f"""
당신은 회계법인 전문 뉴스 분석가입니다. 경쟁 회계법인(한영EY, 삼정KPMG, Deloitte, 안진회계법인 등) 관련 뉴스를 분석하여 중요한 뉴스만 선별해주세요.
...
**URL 기반 판정**
- originallink가 있으면 그 도메인/경로 기준으로 동일 이슈 여부를 판단
- 네이버 기사(news.naver.com, n.news.naver.com, mnews.naver.com)는 article/<oid>/<aid> 조합이 같으면 같은 이슈
- 리다이렉트/미러 URL은 원문 도메인 기준으로 묶음

**제목/내용 기반 판정 (정규화 후 유사도)**
- 제목에서 대괄호, 중복 공백, 따옴표, 이모지, 특수문자 제거 → 영문은 소문자화
- 날짜·수치(%, 원, 억/조 단위)·"단독/속보/영상/종합" 등의 군더더기는 제거
- 정규화된 제목끼리 의미가 동일하거나 핵심명사(회사/사건/역할) + 동일 시점(발표/행사/사건 날짜) 이 같으면 같은 이슈로 본다

**이슈당 1건 선택 기준 (타이브레이커)**
- 기사 품질: 수치·인용·맥락 설명이 더 풍부한 기사
- 출처 신뢰도:
  - 우선순위: 조선일보 > 중앙일보 > 동아일보 > 한국경제 > 매일경제 > 연합뉴스
  - 그 외: 이데일리, 아시아경제, 뉴스핌, 뉴시스, 헤럴드경제, 더벨, 비즈니스포스트, 머니투데이 등
- 최신성: 최초 단독보다 가장 최신이면서 정리·분석이 잘 된 기사
- 동률이면 원문(출처 명확) 기사 우선

⚠️ 결과 목록에는 같은 이슈를 2건 이상 넣지 말 것. 애매하면 가장 좋은 1건만 남긴다.
...
- 기업 구조/가치 변동 (M&A, 자회사 설립·매각, 지분 변동, 신규사업, 전략 방향성 변화, 공급망 이슈 등)
- 산업 동향 (정책, 규제, 시장 변화, 지정학 리스크 등)
...
- 단순 언급/경력 소개/배경 문장 수준
- 광고성·스폰서·외국어 기사
- 중복 보도(→ 이슈당 1건 원칙으로 정리)

**언론사 신뢰도 기준**
- 우선 선별: 조선일보, 중앙일보, 동아일보, 한국경제, 매일경제, 연합뉴스
- 보조 선별: 이데일리, 아시아경제, 뉴스핌, 뉴시스, 헤럴드경제, 더벨, 비즈니스포스트, 머니투데이
- 제외: 블로그, 개인 미디어, SNS, 광고성 콘텐츠, 출처 불분명 기사
